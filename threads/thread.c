...
/* Mon */
	if (thread_mlfqs) {
		...
		if (got_timer_tick % 100 == 0) {
			int64_t plus = (((int64_t)(((rthreads) << 14))) * ((1 << 14)) / (((60) << 14))); 
		...
		int64_t cal_mod = got_timer_tick % 4; 
		if ( cal_mod == 3) {
...
thread_block (void) {
	ASSERT (!intr_context ());
	ASSERT (intr_get_level () == INTR_OFF);
	thread_current ()->status = THREAD_BLOCKED; //mon
	schedule (); //mon
}
...
thread_set_priority (int new_priority) {
	...
	if ((new_priority > current -> donated_priority) || (list_empty (&current->locks)))
